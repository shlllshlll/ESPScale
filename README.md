...
> **v0.3.0** — 基于 ESP32-C3 + HX711 的物联网称重系统，支持 BLE 配网、本地称重与远程数据上传。

## 架构概览

```
┌──────────────────┐     MQTT (JSON)      ┌──────────────────────────┐
│  ESP32-C3 设备    │ ──────────────────> │  FastAPI 单进程           │
│  (HX711 称重)     │  espscale/{id}/     │  · amqtt 内嵌 Broker      │
│                   │    weight | status  │  · MQTT Bridge            │
│  模式A: MQTT远程   │                     │  · SQLite / REST / WS     │
│  模式B: BLE本地秤  │     HTTP POST       │                          │
│  模式C: HTTP直传   │ ──────────────────> │  端口 8000 + 1883        │
└────────┬─────────┘                     └────────────┬─────────────┘
         │ BLE GATT (JSON)                            │
         │ 配网/配置/校准/命令                          │ HTTP/WebSocket
         ▼                                            ▼
┌──────────────────┐                     ┌──────────────────┐
│  APP 端           │◄────────────────────┤  同上 FastAPI     │
│  (Flutter 6平台)  │                     └──────────────────┘
...
│   ├── mqtt_broker.py     # 内嵌 amqtt Broker（无需 Mosquitto）
│   ├── mqtt_bridge.py     # MQTT → DB → WebSocket 桥接
│   ├── retention.py       # 数据保留清理
│   ├── alembic/           # 数据库迁移
│   ├── routes/
│   │   ├── devices.py     # 设备注册 + CRUD
│   │   ├── data.py        # 数据摄入 + 查询 + 统计 + CSV
...
│   └── pubspec.yaml
│
├── docs/                  # 开发部署 / 构建 / 固件 / 变更记录
├── secrets.template/      # 密钥模板（复制到 secrets/）
├── plans/                 # 各模块详细实施计划
├── scripts/               # 开发工具脚本
└── CLAUDE.md              # 技术选型与开发规范
```
...
- Python 3.11+（服务端，**无需 Docker / Mosquitto**）
...
# 可选: set -a && source ../secrets/server.env && set +a
uvicorn main:app --host 0.0.0.0 --port 8000
# API: http://localhost:8000/docs   MQTT: :1883（进程内 amqtt）
```

详见 [docs/environment.md](docs/environment.md) §5。
...
| 服务端 | Python FastAPI, SQLAlchemy 2.0 (async), aiosqlite, amqtt, python-jose | 单进程直接运行 |
| APP 端 | Flutter 3.x, flutter_blue_plus, fl_chart, Riverpod, dio, web_socket_channel | 6 平台 |
| 消息 | MQTT (amqtt 内嵌), WebSocket, HTTP REST | JSON 编码 |
| 部署 | `uvicorn main:app` | 无需 Docker / 外部 Broker |
...
- [x] **单进程服务端** — 内嵌 amqtt + FastAPI（`uvicorn`，无 Docker）
...
| [docs/environment.md](docs/environment.md) | 开发与部署（固件 / 服务端 / APP，无 Docker） |
| [docs/build.md](docs/build.md) | 多端编译与发布（固件烧录 / Flutter 打包） |
